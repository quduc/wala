...
const listLibNeedPatch = [
  {
    pathFile: 'node_modules/react-native-swipeable/lib/index.js',
    version: '0.6.0',
...
for (const lib of listLibNeedPatch) {
  if (
    packageJson.dependencies[lib.name] &&
    packageJson.dependencies[lib.name] === lib.version
  ) {
    if (fs.existsSync(lib.pathFile)) {
      fs.writeFile(lib.pathFile, lib.contentPatch, 'utf8', err => {
        if (err) return console.log(err);
        console.log(`Patch lib ${lib.name} version ${lib.version} success \n`);
      });
    }
  }
}
